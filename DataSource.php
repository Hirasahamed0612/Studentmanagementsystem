...

{
    ...
    public function  logIn($table,$paramType, $paramArray){
        ...
        $checkQuery = "SELECT * FROM $table WHERE $keys[0]=? and $keys[1]=?";
        $stmt = $this->conn->prepare($checkQuery);
      
        $stmt->bind_param($paramType, $values[0],$values[1]);
        $stmt->execute();
    ...
    public  function getCurrency(){
        if(empty($_SESSION)) // if the session not yet started 
            session_start();;
         $result=$this->select("SELECT a.country,b.currency FROM personal_details a inner join countries b on a.country=b.cn_id where a.user_id='{$_SESSION['user'][3]}';");
