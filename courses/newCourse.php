              ...
              <div class="form-group">
                <label>Head of Course :</label>
                 <select class="form-control" name="lecturer" id="lecturer">
                  ...
                  <?php
                  $result=$db->select("SELECT * FROM lecturers where ins_id='{$_SESSION['user'][4]}' ORDER BY ins_id ASC;");
                  if (!empty($result)) {
...
var table;
var images=[];
$("#courses a:eq(0),#courses a:eq(1)").addClass("active");
...
//form submission
    $('#newForm').bootstrapValidator({
      ...
      var data=new FormData(this);
        ...
        $.ajax({
          url: '<?php echo $base_url;?>/courses/vendor/newCourse',
          type: 'POST',
          data: data,
